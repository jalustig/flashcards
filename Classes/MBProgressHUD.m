...
- (void)hide:(BOOL)animated {
    ...
    if (syncHUD && controller) {
        if ([self isEqual:syncHUD]) {
            if (controller.isCurrentlyDownloading && [self.labelText isEqual:NSLocalizedStringFromTable(@"Downloading Database", @"Sync", @"")]) {
                return;
            }
            if (controller.isCurrentlyUploading && [self.labelText isEqual:NSLocalizedStringFromTable(@"Uploading Database", @"Sync", @"")]) {
                return;
            }
        }
    }
