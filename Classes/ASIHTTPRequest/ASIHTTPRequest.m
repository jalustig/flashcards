...
- (void)updateProgressIndicators
{
    ...
    if (![self mainRequest]) {
        if ([self showAccurateProgress] || YES || ([self complete] && ![self updatedProgress])) {
...
- (void)updateDownloadProgress
{
    ...
    // NSLog(@"received x: %d", (int)value);
    if ([self delegate]) {
        // FCLog(@"Received: %d (of total: %d)", value, [self contentLength]);
        if (value >= [self contentLength]) {
            int i = 0;
        } else if ([[self delegate] respondsToSelector:@selector(request:didReceiveBytes:)]) {
            [[self delegate] request:self didReceiveBytes:value];
        }
        if ([[self delegate] respondsToSelector:@selector(request:bytesReceivedSoFar:)]) {
            [[self delegate] request:self bytesReceivedSoFar:bytesReadSoFar];
        }
    }
